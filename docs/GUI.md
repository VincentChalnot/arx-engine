...
| `window_icon.rs` | **Generated**, Linux only — the window/taskbar icon as an ARGB buffer. Do not edit by hand |
| `platform_icon.rs` | Hands that buffer to X11's window-icon API on Linux; a no-op on Windows/macOS, where the icon comes from packaging instead (see below) |
...
`assets/pixel/logo.xcf` is the source for two separate icons, generated by
two separate scripts because they solve different problems:

- **Window/taskbar icon** (Linux/X11 only): `scripts/gen_window_icon.py`
  centers the 46×37 crest — unscaled, since nearest-neighbor downscaling
  would mangle art drawn pixel by pixel — on a 64×64 transparent square and
  writes it to `src/gui/window_icon.rs` as a flat
  `[width, height, argb pixels...]` buffer. "On" pixels are opaque
  `COL_GOLD`, everything else fully transparent, so the icon reads against
  any desktop theme. `platform_icon::set_window_icon` passes this straight
  to minifb's `Icon: TryFrom<&[u64]>` (a `_NET_WM_ICON` buffer — its
  element type and layout are exactly this format).
- **App icon** (Windows `.exe` resource, macOS `.app` bundle):
  `scripts/gen_app_icon.py` block-scales the same crest with nearest-
  neighbor pixel replication onto a padded, opaque `COL_PAGE_BG` square
  (an app icon needs its own backdrop; the window icon doesn't, since it
  sits against the desktop) and writes `assets/generated/keres.ico` /
  `keres.icns`.

Installing the window icon is per-OS because minifb's `Icon` is three
different things, and only one of them is worth using at runtime:

| Platform | minifb API | What we do |
|---|---|---|
| Linux/X11 | `Icon: TryFrom<&[u64]>` — a `_NET_WM_ICON` ARGB buffer | Pass the generated buffer straight through |
| Windows | `Icon::from_str` only — a path to an `.ico` **file on disk** | Nothing at runtime: `build.rs` embeds `keres.ico` as a PE resource, which Explorer/taskbar/Alt-Tab read directly from the `.exe` with no window-icon call needed |
| macOS | none — `set_icon` is `unimplemented!()` and panics | Nothing at runtime; the Dock icon is `keres.icns` in the `.app` bundle's `Info.plist`, assembled by `scripts/package_macos_app.sh` |

This used to be one unsafe `WM_SETICON`/`HICON` block built from the window
buffer at startup, which was the GUI's only `unsafe` code; embedding the
`.ico` as a build-time PE resource replaced it outright — see
`src/gui/platform_icon.rs`.
